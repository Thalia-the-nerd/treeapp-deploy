document.addEventListener("DOMContentLoaded", () => {
  ...
  const userActivityContainer = document.getElementById(
    "user-activity-table-container",
  );
  ...
  const loadUserDetails = async () => {
    try {
      ...
      if (!user) return; // Stop if fetchData redirected
      userDetailsTitle.textContent = `User Details: ${user.username}`;
      userInfoContainer.innerHTML = `
        <p><strong>Username:</strong> ${user.username}</p>
        <p><strong>Email:</strong> ${user.email}</p>
        <p><strong>Password:</strong> ${user.password}</p>
        <p><strong>Money Donated:</strong> ${(user.moneyDonated || 0).toFixed(
          2,
        )}</p>
        <p><strong>Time Donated:</strong> ${(user.timeDonated || 0).toFixed(
          1,
        )} hours</p>
  ...
  const loadUserActivity = async (filter = "") => {
    try {
      ...
      if (!activity) return; // Stop if fetchData redirected
      let filteredActivity = activity;
      if (filter) {
        filteredActivity = activity.filter((log) =>
      ...
      userActivityContainer.innerHTML =
        '<p class="error">Could not load user activity.</p>';
    }
  };

  document
    .getElementById("view-login-history-btn")
    .addEventListener("click", () => loadUserActivity("login"));
  ...
  document.getElementById("send-warning-btn").addEventListener("click", () => {
    openModal(
      "Send Warning",
      '<textarea name="message" rows="5" required placeholder="Warning message..."></textarea>',
      (formData) => {
        const message = formData.get("message");
        handleAction(
          (url) => postData(url, { message }),
          `/api/users/${userId}/send-warning`,
          "Warning sent.",
        );
      },
    );
  ...
  document.getElementById("assign-badge-btn").addEventListener("click", () => {
    openModal(
      "Assign Badge",
      '<input type="text" name="badge" required placeholder="Badge name...">',
      (formData) => {
        const badge = formData.get("badge");
        handleAction(
          (url) => postData(url, { badge }),
          `/api/users/${userId}/assign-badge`,
          "Badge assigned.",
        );
      },
    );
  ...
  document.getElementById("add-note-btn").addEventListener("click", () => {
    openModal(
      "Add Note",
      '<textarea name="note" rows="5" required placeholder="Note..."></textarea>',
      (formData) => {
        const note = formData.get("note");
        handleAction(
          (url) => postData(url, { note }),
      ...
      openModal(
        ...
        (formData) => {
          ...
          handleAction(
            ...
            `/api/users/${userId}/change-username`,
            "Username changed.",
          );
        },
      );
  ...
  document.getElementById("anonymize-user-btn").addEventListener("click", () => {
    if (
      confirm(
        "Are you sure you want to anonymize this user? This will remove their personal information and cannot be undone.",
      )
    ) {
      handleAction(
        postData,
  ...
  placeholderButtons.forEach(id => {
    ...
    if (button) {
      button.addEventListener("click", () => alert("Functionality not yet implemented."));
    }
  });
