...
document.addEventListener("DOMContentLoaded", () => {
    ...
    (entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          ...
          if (treeInfo) {
            anime({
              targets: treeInfo.querySelectorAll("h3, .scientific-name, .stat, .tree-description, .tree-actions"),
              translateX: [-30, 0],
              opacity: [0, 1],
              duration: 800,
  ...
  if (plantButton) {
    plantButton.addEventListener("click", () => {
      ...
      showNotification(
        "Thank you for your interest in planting a Live Oak! We will contact you with next steps.",
        "success",
      );
    });
  }
  ...
  const treeDescriptionItems = treeFeature.querySelectorAll(
    ".tree-description li",
  );
